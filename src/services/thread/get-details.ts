...
type GetInteractionsType = CAHandlerNoAuth<{params: {did: string, rkey: string, collection: string}, query: {limit?: string, cursor?: string}}, {profiles: ProfileViewBasic[], cursor?: string}>
...
type GetQuotesType = CAHandlerNoAuth<{params: {did: string, rkey: string, collection: string}, query: {limit?: string, cursor?: string}}, {posts: PostView[], cursor?: string}>
...
export const getQuotes: GetQuotesType = async (ctx, agent, {params, query}) =>  {
    ...
    const data = {posts: uris.map(d => hydratePostView(ctx, d, dataplane).data).filter(x => x != null),
        cursor : cursor
    }
    return {data}
}