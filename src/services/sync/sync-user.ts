...
export async function syncAllUsers(ctx: AppContext, mustUpdateCollections?: string[], retries: number = 100, ignoreStatus: boolean = false) {
    let users: string[]
    if(ignoreStatus){
        const {users: userViews, error} = await getUsers(ctx)
        if(userViews){
            users = userViews.map(u => u.did)
        } else {
            return {error}
        }
    } else {
