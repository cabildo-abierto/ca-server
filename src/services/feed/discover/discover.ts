...
import {min} from "#/utils/arrays.js";
...
const getDiscoverFeedSkeleton: GetSkeletonProps = async (ctx, agent, data, cursor) => {
    ...
    /*const skeleton = await ctx.kysely.selectFrom("Record")
        .select(["Record.uri as contentId", "Record.created_at_tz as created_at"])
        ...
        .where("Record.created_at_tz", "<", firstFetchDate)
        .where("Record.created_at_tz", "<", dateSince)
